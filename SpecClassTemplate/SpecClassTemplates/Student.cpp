...
Student::Student(const std::tuple<const char*, std::initializer_list<size_t>, Gender>& tuple)
{
	cout << "---------Start---Tuple---------" << endl;
	id = ++countStudents;
	if (this->name != nullptr)
	{
		delete[] this->name;
	}
	size_t size = strlen(get<0>(tuple)) + 1; // + 1   \0
	this->name = new char[size];
	strcpy_s(this->name, size, get<0>(tuple));

	marks = new size_t[size = get<1>(tuple).size()];
	for (size_t i = 0; i < size; i++)
	{
		marks[i] = *(get<1>(tuple).begin() + i);
	}
	qtyMarks = get<1>(tuple).size();
	setGender(get<2>(tuple));
}
Student::Student()
	: Student("Noname")
{}
